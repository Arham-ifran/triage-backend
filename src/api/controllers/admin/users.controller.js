...
const Kycmdoel = require("../../models/kyc.model");
...
exports.list = async (req, res, next) => {
  try {
    let { all, page, limit, firstName, lastName, kyc, startDate, endDate } =
      req.query;
    let { email } = req.body;
    const filter = {};
    ...
    if (kyc) {
      if (kyc === "true") {
        filter["kycs.appliedKYC"] = 3;
      } else if (kyc === "false") {
        filter["$or"] = [{ "kycs.appliedKYC": 2 }, { "kycs.appliedKYC": 1 }];
      }
    }
    ...
    limit = limit !== undefined && limit !== "" ? parseInt(limit) : 10;
      ...
      pipeline1.push({
        $project: {
          ...
          lastName: 1,
          email: 1,
          kycStatus1: "$kycs.appliedKYC",
          createdAt: 1,
          status: 1,
        },
      });

      const count = await User.aggregate(pipeline1);
...
exports.create = async (req, res, next) => {
  try {
    ...
    if (username && email && password) {
      ...
      let content = {
        "${url}": `${process.env.REACT_APP_FRONT_URL}/verify-email/${rndmoken}`,
        "${email}": email,
      };
      const payload = {};
