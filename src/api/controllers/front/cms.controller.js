...
exports.list = async (req, res, next) => {
    try {
        let cmsFilter = { $and: [{status: true, slug: {$nin: ["privacy-policy","terms-and-conditions"]}}]}
        let cms = await Cms.find(cmsFilter, { title: 1, slug: 1, _id: 0 }).sort({ createdAt: -1 }).limit(5);
