...
exports.fetchDynamicPagesData = async (req, res, next) => {
  try {
    let privacyPolicyData = await cms.findOne({status: true, slug: 'privacy-policy'})
    let termsAndConditionsData = await cms.findOne({status: true, slug: 'terms-and-conditions'})
    return res.send({ success: true, message: "Dynamic Pages Data Retrieved Successfully.", data: {privacyPolicyData,termsAndConditionsData} });
  } catch (error) {
      return next(error);
  }
}
